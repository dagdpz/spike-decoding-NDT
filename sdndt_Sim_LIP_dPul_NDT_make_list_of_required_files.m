...
for i = 1:length(files)
    ...
    % Prepend 'S' to the session ID to ensure it starts with a letter
    %sessionID = ['S', sessionID];
    
   % Extract the last two characters from unitID
    unitIDSuffix = unitID;
    
    % Replace non-alphanumeric characters in sessionID and unitID with underscores
%     sessionID = regexprep(sessionID, '[^a-zA-Z0-9]', '_');
%     unitID = regexprep(unitID, '[^a-zA-Z0-9]', '_');
    
    % Create a unique field name for the unit using the last two characters
    unitFieldName = ['Unit_', unitIDSuffix];
...
%         unitFieldName = ['Unit_', unitID];
%         unitFieldName = regexprep(unitFieldName, '[^a-zA-Z0-9]', '_'); % Replace problematic characters
%         sessionBlockInfo.(sessionID).unitConventions.(unitFieldName) = data.raster_site_info.block_unit(2, :);
       sessionBlockInfo.(sessionID).unitConventions.(unitFieldName) = data.raster_site_info.block_unit(2, :);
        
        
        
        % Check if it's the first block
        if any(contains(data.raster_site_info.block_unit(1, :), '1')) && all(data.raster_labels.block{1} == 1)
            list_of_required_files.firstBlockFiles = [list_of_required_files.firstBlockFiles, files(i).name];
            
            
            % Check if it's the second block
        elseif any(contains(data.raster_site_info.block_unit(1, :), '2'))
        ...
        elseif any(contains(data.raster_site_info.block_unit(1, :), '3'))
...
for s = 1:numel(sessions)
    ...
    if areConventionsSame(sessionBlockInfo.(sessionID).unitConventions, unitsWithMaxBlocks, sessionBlockInfo.(sessionID).maxBlock)
        ...
        for unitID = unitsWithMaxBlocks
            ...
            %commonFiles = getCommonFiles(sessionID, unitID, files);
...
%     unitsWithMaxBlocks = {};
%     for unitID = fieldnames(unitConventions)'
%         unitID = unitID{1};
%        % if isfield(unitConventions.(unitID), num2str(maxBlock))
%         if isfield(unitConventions.(unitID), ['Block_', num2str(maxBlock)])  % Updated field name
...
%         unitID = unitID{1};
%         
%         % Debugging: Display information about the unit
%         disp(['Unit ID: ', unitID]);
%         disp(unitConventions.(unitID));
%         
%         % Check if the field exists for the current unit
%         if isfield(unitConventions.(unitID), ['Block_', num2str(maxBlock)])
%             unitsWithMaxBlocks = [unitsWithMaxBlocks, unitID];
%         end
%     end
% end
...
function unitsWithMaxBlocks = getUnitsWithMaxBlocks(unitConventions, maxBlock)
    ...
    for idx = 1:numel(fieldNames)
        ...
        if iscell(unitData) && size(unitData, 2) == maxBlock
            unitsWithMaxBlocks = [unitsWithMaxBlocks, unitID];
        end
    end
end
...
%         referenceUnitID = unitsWithMaxBlocks{1};
%         referenceConventions = unitConventions.(referenceUnitID).(['Block_', num2str(maxBlock)]);
%         
%         for p = 2:numel(unitsWithMaxBlocks)
%             unitID = unitsWithMaxBlocks{p};
%             if ~isequal(referenceConventions, unitConventions.(unitID).(['Block_', num2str(maxBlock)]))  
...
% function commonFiles = getCommonFiles(sessionID, unitID, files)
%     commonFiles = {};
%     for w = 1:length(files)
%         [~, fileSessionID, fileUnitID, fileBlockNumber, fileType] = extractFileInfo(files(w).name);
%         if strcmp(sessionID, fileSessionID) && strcmp(unitID, fileUnitID) && fileBlockNumber <= sessionBlockInfo.(sessionID).maxBlock
...
function commonFiles = getCommonFiles(sessionID, unitID, files, sessionBlockInfo)
    ...
    for w = 1:length(files)
        ...
        if strcmp(sessionID, fileSessionID)
            ...
            disp(['file: ', files(w).name]);
            disp(['numericUnitID: ', numericUnitID]);
            
            % Check if the numeric part of unitID matches fileUnitID
            if strcmp(numericUnitID, fileUnitID)
                commonFiles = [commonFiles, files(w).name];
            end
        end
    end
...
function numericUnitID = extractNumericUnitID(unitID)
    ...
    numericUnitID = regexprep(unitID{1}, '\D', ''); 
end
